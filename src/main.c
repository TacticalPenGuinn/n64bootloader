...
void init_n64(void)
{
	...
	display_init((resolution_t){640, 240, false}, DEPTH_16_BPP, 2, GAMMA_NONE, ANTIALIAS_RESAMPLE);
...
//static const char * const args[] = { 	(const char *) hdrbuf,
//					(const char *) hdrbuf + 128 };

static const char * const args[] = {	(const char *) 0xffffffff, (const char *) hdrbuf,
...
int main(void)
{
	...
	sprintf(buf, "Found %u kb of RAM", osMemSize / 1024);
	printText(disp, buf, 4, 3);
	
	u32 kernelsize __attribute__((aligned(8)));
	u32 disksize __attribute__((aligned(8)));
	u32 diskoff __attribute__((aligned(8)));
	...
	sprintf(buf, "Address: %p", ptr);
	printText(disp, buf, 4, 5);
	...
	sprintf(buf, "LoadAddress: %p", (void *) phdr->p_paddr);
	printText(disp, buf, 4, 6);

	sprintf(buf, "LoadOffset: %p", (void *) 0xB0101000 + phdr->p_offset);
	printText(disp, buf, 4, 7);
	...
	sprintf(buf, "Entry: %p", (void *) ptr->e_entry);
	printText(disp, buf, 4, 8);
	...
	sprintf(buf, "Disk: %u", diskoff);
	printText(disp, buf, 4, 9);
	printText(disp, (const char *)hdrbuf, 4, 10);
	printText(disp, (const char *)hdrbuf+128, 4, 11);
	
	sprintf(buf, "Jumping: %p", (void *) funcptr);
	printText(disp, buf, 4, 1);

	delay(100);
	display_close();
	display_init((resolution_t){320, 240, false}, DEPTH_16_BPP, 2, GAMMA_NONE, ANTIALIAS_RESAMPLE);
	disable_interrupts();
